...
func TestWarnIfConfigReferences_PathEntry(t *testing.T) {
	extDir := setupExtensionsDir(t)

	// Create the extension to uninstall.
	extPath := filepath.Join(extDir, "my-tool")
	require.NoError(t, os.MkdirAll(extPath, 0o750))
	require.NoError(t, os.WriteFile(filepath.Join(extPath, "main.go"), []byte("package main\n"), 0o600))

	// Create a config file that references the extension via path.
	projectDir := t.TempDir()

	configContent := "extensions:\n  - " + extDir + "/my-tool\n"
	require.NoError(t, os.WriteFile(filepath.Join(projectDir, ".weave.yaml"), []byte(configContent), 0o600))

	// Change to project dir so config discovery finds it.
	oldWd, _ := os.Getwd()
	...
	// Run uninstall — should succeed and warn about the path-based reference.
	code := runUninstall([]string{"my-tool"})
	assert.Equal(t, 0, code)
}

func TestWarnIfConfigReferences_BareName(t *testing.T) {
	homeDir := t.TempDir()
	t.Setenv("HOME", homeDir)

	extDir := filepath.Join(homeDir, ".weave", "extensions", "my-tool")
	require.NoError(t, os.MkdirAll(extDir, 0o750))
	require.NoError(t, os.WriteFile(filepath.Join(extDir, "main.go"), []byte("package main\n"), 0o600))

	// Create a config file that references the extension by bare name.
	projectDir := t.TempDir()

	require.NoError(t, os.WriteFile(filepath.Join(projectDir, ".weave.yaml"), []byte("extensions:\n  - my-tool\n"), 0o600))
