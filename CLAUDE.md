...
- **Manual compaction**: triggered via `/compact [instructions]` slash command, which sends an `agent.steer` event with `"compact"` prefix. In `waitForInput` state, compaction runs non-fatally (errors are published as `agent.compacted` with error field). In the inner loop, auto-compaction errors are fatal (session ends).
