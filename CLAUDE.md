...
`sdk.Config` is the carrier interface (`FilePath()`, `ProjectDir()`, `ExtensionConfig(scope, name, target, envPrefix)`, `IsHeadless()`, `Preferences()`, `SavePreferences()`, `SaveProviderKey()`, `RespectGitignore()`) — extensions receive populated config structs in their factory functions. `IsHeadless()` returns true when running without TUI (print mode); extensions use it to skip UI-dependent work. `HeadlessConfig` wraps a `Config` and overrides `IsHeadless()`. `wire.WireWithCore` takes a `wire.CoreWireConfig` struct (AgentLoop + SingleTurn) without provider or extension lists.
