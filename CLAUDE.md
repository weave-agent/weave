...
A coding agent framework written in Go — event-driven, extension-based, with dynamic compilation of selected extensions at runtime. Agent-loop, providers (Anthropic, OpenAI, Z.ai, Kimi, Codex), tools (bash, read, edit, write, grep, find, ls, subagent), and a terminal UI (TUI) are implemented as independent extension modules.
...
**Key packages:**
- `sdk/` — defines `Extension`, `Bus`, `Config`, `UI` interfaces; `Handler func(Event) error` type for callback-based bus handlers; `Event` struct carries `Topic`, `Payload`, `Timestamp`, and `TraceID` (16-byte random hex for distributed tracing); `Config` includes `FilePath()`, `ProjectDir()`, `ExtensionConfig(scope, name, target)`, `IsHeadless()`, and `RespectGitignore() bool`; `PreferenceReader` interface with `Preferences(target)`; `PreferenceWriter` interface extending `PreferenceReader` with `SavePreferences(target)` and `SaveProviderKey(provider, key)`; `PreferenceStore` is the full read/write interface (backward compatible); `HeadlessConfig` wraps a `Config` and overrides `IsHeadless()`; `NoopConfig` is a nil-safe Config stub that returns empty/zero values; `NoopPreferenceStore` is a nil-safe PreferenceStore stub; `configOrDefault(cfg)` returns the given Config or a `NoopConfig` stub if nil; `preferenceStoreFrom(cfg)` extracts `PreferenceStore` from a `Config` or returns `NoopPreferenceStore`; global registries for extensions, providers, tools, and UIs (`RegisterExtension`/`GetExtension`, `RegisterProvider`/`GetProvider`, `RegisterTool`/`GetTool`/`GetToolFilter`, `RegisterUI`/`GetUI`) with duplicate registration warnings (first wins, logs to stderr); `RegisterUIExtension`/`GetUIExtensions`/`ResetUIExtensionRegistry` for TUI-specific plugin detection; `Sandboxer` interface (`WrapCommand`, `AllowWrite`, `AllowRead`); `OnBusReady(fn)` / `InvokeBusSubscribers(bus)` for tool bus registration; `OutputRedirectPayload` event payload for `output.redirect` bus events; `ErrNotRegistered` sentinel error; `Message` types; `ProviderUsage` struct (InputTokens, OutputTokens, CacheCreationTokens, CacheReadTokens) and `ProviderEventUsage` constant for token usage events; `NoopUI` stub for headless mode; `FileTracker` interface (`RecordRead`, `WasRead`, `GetReadTime`) with global getter/setter (`SetFileTracker`/`GetFileTracker`) for read-before-edit enforcement; `FileMuter` interface (`Lock(path) func()`) with global getter/setter (`SetFileMutex`/`GetFileMutex`) for per-file mutation serialization; `WithBus(ctx, bus)`/`BusFromContext(ctx)` to attach/retrieve the event bus from context for tool streaming; `ReadDonePayload`, `BashOutputPayload`, `BackgroundStartPayload`, `BackgroundDonePayload` event payload types for tool bus events; `Logger(name)` returns a structured `*slog.Logger` pre-tagged with `"ext": name` for extension diagnostic output; `SessionStore` interface (`ListSessions()`, `LoadHistory(sessionID)`) with `SessionInfo{ID, CWD, CreatedAt, UpdatedAt}` struct and global getter/setter (`SetSessionStore`/`GetSessionStore`) following the `FileTracker`/`FileMuter` pattern; `SessionResumePayload{SessionID, Messages}` for `session.resume` bus events; `NoopSessionStore` zero-value stub; `sdk/auth.go` — exports `OAuthCredential`, `DeviceCodeResponse`, `TokenResponse` types and helpers `ClearProviderAuth`, `SetOAuthCredential`, `RefreshOAuthTokenIfNeeded`, `RunAuthorizationCodeFlow`, `RequestDeviceCode`, `PollDeviceToken`; `sdk/oauth.go` — OAuth provider registry (`RegisterOAuthProvider`, `GetOAuthProvider`, `ListOAuthProviders`) with `OAuthFlowType` (`AuthorizationCode`, `DeviceCode`) and `OAuthProvider` struct
- `sdk/ui.go` — `UI` interface composed of three sub-interfaces: `UIDialogs` (`Select`, `Confirm`, `Input`, `MultiSelect`, `Editor` — all with variadic functional options), `UIStatus` (`SetStatus`, `Notify`, `NotifyTyped`, `ShowError`, `SetWorking`, `ClearWorking`), and `UIRegistry` (`RegisterCommand`, `RegisterRenderer`, `RegisterKeybinding`, `SetTheme`, `ListThemes`). Functional options: `SelectOption`, `ConfirmOption`, `InputOption`, `EditorOption`, each with a docked-placement variant (`WithKeepContent()` for selects, `WithKeepContentConfirm()` for confirms, `WithKeepContentInput()` for inputs, `WithKeepContentEditor()` for editors) that renders the overlay at the bottom of the chat area (12 rows) instead of a centered modal, keeping chat history visible. `WithMask(rune)` sets a mask character (e.g. `'*'`) for password input on `Input()`. `NotifyLevel` enum (`NotifyInfo`, `NotifyWarning`, `NotifyError`, `NotifySuccess`) for typed notifications. `ThemeInfo` struct provides read-only theme colors for extensions, including dynamic accent fields (`Accent`, `AccentDim`, `AccentBright`) and state-specific background tints (`BackgroundTintPending`, `BackgroundTintSuccess`, `BackgroundTintError`). `NoopUI` provides zero-value defaults for all methods (returns first item for selects, `true` for confirms, empty strings/zeros for everything else)
...
- `sdk/retry/` — shared retry utility with configurable max retries (default 10), exponential backoff (1s base, 2x multiplier, 30s cap), and retriable-error predicate. Used by providers for automatic retry on rate limits, server errors, and network failures
- `sdk/validate/` — lightweight JSON schema validator for tool argument validation. Checks required fields, type conformance, and rejects unknown properties when `additionalProperties: false` is set on tool schemas
...
- `internal/auth/` — provider credential storage in `~/.weave/auth.json`; supports both API keys (`api_key`) and OAuth tokens (`access_token`, `refresh_token`, `expires_at`, `token_type`). `Load()`/`Save()`/`SetProviderKey()`/`GetProviderKey()`/`SetOAuthCredential()`/`GetOAuthCredential()`/`ClearProviderAuth()`/`RefreshOAuthToken()`/`RefreshOAuthTokenIfNeeded()`; `LoadProviderAuth(providerName, target)` loads auth into a typed struct from auth.json + env vars (using `env` tags with no prefix). `callback_server.go` — temporary localhost callback server for authorization code flows. `oauth_flow.go` — PKCE generation, authorization URL building, browser opening, token exchange, device code request/polling. `config.go` — `LoadProviderAuth` with env tag application and OAuth credential bridging from flat auth.json fields to nested struct fields. Used by the `sdk.RegisterProvider` wrapper for automatic auth injection into provider factories. Auth file permissions are `0600`
...
- `extensions/tools/subagent/` — subagent tool extension; spawns isolated subagent subprocesses via the weave launcher (`WEAVE_LAUNCHER_PATH` env var) with `--output=json` and `--weave-prompt-file` flags, running in headless mode. The launcher ensures the subagent binary includes all registered extensions. Agent definitions are markdown files with YAML frontmatter (name, description, tools, model, sandbox, messaging, system) discovered from embedded built-ins (`agents/general.md`, `explore.md`, `plan.md`), `.weave/agents/`, and `~/.weave/agents/` (project > global > built-in precedence). Each discovered agent registers as `subagent_<name>` tool. Three execution modes: single (`prompt`), parallel (`tasks`), chain (`chain` with `{previous}` substitution). Background mode spawns without blocking; `check_agent(id)` and `await_agent(id)` query results. Publishes `subagent.started` (payload: `map[string]string` with `id`, `name`, `mode`) when a background agent spawns, `subagent.done` (payload: `map[string]string` with `id`, `status`, `content`) on background agent completion, and `subagent.output` (payload: `map[string]string` with `id`, `type`, `tool`, `content`) for each parsed JSON event from child stdout, enabling live output streaming to the UI. Per-agent cancellation via dedicated `context.CancelFunc` stored on each `backgroundAgent`; subscribes to `subagent.cancel` bus event (payload: `map[string]string` with `id`) to cancel individual running agents. Inter-agent communication via parent `Broker` routes `send_message`, `broadcast_message`, and `list_agents` between children when `messaging: true`. Broker registers agents by ID, monitors stdout for JSON routing events, and writes `agent_msg`/`inject`/`list_agents_response` to target stdin pipes. Child-side `stdin_listener.go` reads stdin JSON lines and queues them as user messages when `--weave-subagent-id` is set. Subagent uses `testRunSubagent` hook for mocking in tests
...
- `extensions/providers/{anthropic,openai,zai,kimi,codex}/` — provider extension modules; Anthropic and Kimi use official SDKs (Kimi uses Anthropic SDK with custom base URL), OpenAI and Z.ai delegate to `openai-compat`; Codex is OAuth-only and uses the Responses API with SSE streaming
...
- `extensions/ui/tui/` — interactive terminal UI extension built with Bubble Tea v2 (`charm.land/bubbletea/v2`) + Ultraviolet screen buffers for `Draw()` rendering. Key handling uses `tea.KeyPressMsg` (not v1 `tea.KeyMsg`). Mouse support uses `tea.MouseModeAllMotion` to capture click, drag, and release events for in-app text selection; wheel events scroll the chat viewport. Components use `lipgloss.NewStyle()` from `charm.land/lipgloss/v2`. Screen buffer rendering pattern: `Model.View()` creates an `uv.NewScreenBuffer(w,h)`, delegates to `Model.Draw()` which computes layout via `LayoutEngine` then draws each component, and returns `uv.TrimSpace(canvas.Render())`. Components implement `Draw(scr uv.Screen, area uv.Rectangle)` alongside retained `View() string` (used internally for string-to-buffer conversion via `uv.NewStyledString`). Uses `github.com/charmbracelet/ultraviolet` for `uv.Screen`, `uv.Rectangle`, `uv.layout` splitting. `github.com/atotto/clipboard` provides OS clipboard access for text selection copy. Self-registers via `sdk.RegisterExtension("tui", ...)` and `sdk.RegisterUI("tui", ...)`; implements `sdk.UI` interface for cross-extension integration (popups, status bar, slash commands, keybindings); bridge goroutine translates bus events to Bubble Tea messages; includes: streaming chat with progressive markdown rendering (Glamour with custom xchroma formatter), dialog stack for layered overlays, landing state before first prompt, smart auto-scroll with new-content indicator, token rate display in footer, tool output panels, thinking blocks, diff highlighting, multi-line editor (bubbles/v2 textarea) with history, file attachments with paste detection, pills bar for tool progress, slash commands, session/model selectors, and configurable keybindings. UI extensions (universal, via `RegisterUIExtension`) are wired at startup via `sdk.GetUIExtensions()`; TUI-specific extensions (via `RegisterTUIExtension` in the `tui` package, see below) are wired via `sdk.GetTUIExtensions()`
  ...
  - `auth_selector.go` — `buildLoginProviders()` returns all providers available for login (OAuth + API key) with auth status; `buildLogoutProviders()` returns only authenticated providers. `displayNameForProvider()` maps provider IDs to human-readable names.
  - `login_flow.go` — `runOAuthFlowCmd()` executes authorization code OAuth flow asynchronously; `pollDeviceCodeCmd()` polls token endpoint for device code flow. `onLoginFlowResult()` saves credentials, updates `model.SetProviderAuth`, and publishes `auth.login.success` + `model.change` bus events.
  - `panel.go` — `PanelManager` tracks registered panels (`Show`/`Hide`/`Remove`/`PanelVisible`/`Active`); `PanelConfig` holds `ID`, `Placement` (`AsOverlay` renders as floating overlay without consuming layout rows; `AboveEditor`/`BelowEditor` reserve height in layout; `TrayOnly` shows only in the panel tray tab strip without reserving layout rows, expanding to an overlay when activated via Enter), `Blocking`, `Width`, `Height`, `Title` (displayed in panel tray tab, falls back to ID if empty); `PanelDrawer` interface (`Draw`, `Update`, `Handles`) for panel content rendering
  ...
  - `model.go` — OAuth flows are cancellable: `Model.oauthCancel` stores a `context.CancelFunc` that dismisses an in-flight authorization code or device code flow when the user presses Escape or Ctrl+C on the login dialog.
  - `extensions/ui/tui/extensions/subagent-ui/` — TUI extension for subagent panel visualization; subscribes to `subagent.started`, `subagent.done`, `subagent.output`, and `agent.end` bus events via `sdk.OnBusReady`; creates an `AboveEditor` panel for each running background agent showing name, status indicator (●/✓/✗/⊘), mode, and elapsed time; expanded panel view includes scrollable tool log from per-agent `outputRing` buffer (200-entry capacity) and cancel button (Ctrl+X or Enter publishes `subagent.cancel`, only for running agents); panels auto-remove after 3-second grace period on completion; `agent.end` triggers `Close()` to cancel grace-period timers and clear tracked state; registers a `RichToolRenderer` for `subagent_*` tool names to render compact result cards in chat; uses `AgentTracker` for thread-safe agent state with timer-based grace period removal; `AgentStatus` enum includes `AgentCancelled` for cancelled agents; self-registers via `tui.RegisterTUIExtension("subagent-ui", ...)`
...
2. **Available tools** — dynamic list from `sdk.ListTools()` with descriptions.
3. **Skills XML** — `<available_skills>` block with name, description, and file path for each discovered skill.
4. **Skills usage instructions** — `<skills_usage>` block instructing the model to read matching skills via the read tool before acting.
5. **Context files** — `# Project Context` section with CLAUDE.md/AGENTS.md content, wrapped in `<user_context trust="untrusted">` tags to mark user-provided guidance.
6. **APPEND_SYSTEM.md** — always appended last, wrapped in `<user_appended_context>` tags, regardless of whether SYSTEM.md or the default prompt was used as the base.
7. **Date + CWD** — always injected last (`Current date: YYYY-MM-DD`, `Current working directory: /path`). Placed at the end to preserve cacheability of the earlier static layers.
...
Skills are listed in the system prompt as XML with their name, description, and file location. The model is instructed to read matching skills using the read tool before taking other action. When a user invokes a skill via the `/skill:<name>` slash command, the skill body is pre-loaded into the conversation as an `agent.prompt` event wrapped in `<skill_body trust="untrusted">` XML tags. The system prompt instructs the model to treat `<skill_body>` content as user-provided skill content, not system policy.

**AllowedTools enforcement**: When a skill declares `allowed-tools` in its frontmatter, the agent temporarily restricts the available tool set to only those tools during skill execution. The previous tool filter is restored after the skill completes. If `allowed-tools` is empty, no filtering is applied.
...
**Step limit**: The inner loop enforces a configurable `max_steps` limit (default 50). When exceeded, `continueLoop` is set to `false` and an `agent.compacted`-style warning event is published. This prevents infinite tool-call loops from burning tokens.

**Tool execution panic recovery**: `executeTool()` wraps each tool call in a `defer recover()` that returns `ToolResult{Content: "tool panicked: ...", IsError: true}` and logs the stack trace via `sdk.Logger`. This prevents a single buggy tool from crashing the entire agent loop.

**Parallel read-only tool execution**: Read-only tools (`read`, `grep`, `find`, `ls`) execute concurrently via goroutines when multiple tool calls are present in a single turn. Write tools (`edit`, `write`, `bash`, `subagent`) execute sequentially after all reads complete. Results are collected in original tool call order to preserve message sequencing. Classification is by tool name in `isReadOnlyTool()` in `extensions/agent/loop.go`.

**Tool result trust labeling**: Tool result content is wrapped in `<tool_output name="...">` XML tags when constructing `NewToolResultMessage()`. The system prompt instructs the model to treat `<tool_output>` content as external data produced by tools, not system instructions.

**Tool argument validation**: Before executing any tool, incoming `map[string]any` arguments are validated against the tool's `Definition().Parameters` schema. Required fields are checked, types are verified, and unknown properties are rejected when `additionalProperties: false` is set. Validation failures return `ToolResult{Content: "invalid arguments: ...", IsError: true}` without invoking the tool.

**Token usage events**: When a provider emits a `ProviderEventUsage` event at the end of a stream, the agent forwards it as `agent.usage` with payload containing `input_tokens`, `output_tokens`, `cache_creation_tokens`, and `cache_read_tokens`.
...
**`CompactionConfig`** struct: `Enabled` (bool, default true), `ReserveTokens` (int, default 16384), `KeepRecentTokens` (int, default 20000), `Model` (string, empty = current model), `MaxSteps` (int, default 50). Loaded from extension config scope.
...
`sdk.Config` is the carrier interface (`FilePath()`, `ProjectDir()`, `ExtensionConfig(scope, name, target)`, `IsHeadless()`, `RespectGitignore()`) — extensions receive populated config structs in their factory functions. `sdk.PreferenceReader` provides `Preferences(target)` for read-only access; `sdk.PreferenceWriter` extends it with `SavePreferences(target)` and `SaveProviderKey(provider, key)`. Tools and most extensions receive `PreferenceReader` in their factory signatures; only auth-related extensions receive `PreferenceWriter`. `IsHeadless()` returns true when running without TUI (print mode); extensions use it to skip UI-dependent work. `HeadlessConfig` wraps a `Config` and overrides `IsHeadless()`. `wire.WireWithCore` takes a `wire.CoreWireConfig` struct (AgentLoop + SingleTurn) without provider or extension lists.
...
WEAVE_AGENT_MAX_STEPS=50               # Inner loop step limit (default 50)
```
...
--agent-max-steps=50                   # Inner loop step limit (default 50)
```
...
- `CODEX_API_KEY` — API key for Codex provider (default model: `gpt-5.5`, override with `CODEX_MODEL`). OAuth token is also accepted if set via `/login`.
- `CODEX_BASE_URL` — override the default API base URL for Codex provider
...
**Provider retry behavior** — All providers automatically retry on retriable errors (rate limits, server errors, network failures) with exponential backoff: 1s base, 2x multiplier, 30s cap, up to 10 retries. The `sdk/retry/` utility provides `Do(fn, opts)` with `WithMaxRetries(n)`, `WithBackoff(base, cap)`, and `WithRetriable(pred)`. Non-retriable errors (authentication failures, invalid requests) fail immediately.

**Provider token usage** — Providers emit `ProviderEventUsage` bus events at the end of each stream with `InputTokens`, `OutputTokens`, `CacheCreationTokens`, and `CacheReadTokens`. The TUI footer surfaces these values. Anthropic and OpenAI-compatible providers extract usage from the final stream event.

**Prompt caching (Anthropic)** — The Anthropic provider adds `cache_control: {type: "ephemeral"}` to the system prompt content block and the last user message (or the most recent compaction summary message) to enable conversation caching. Cache hit rates can be monitored via `CacheReadTokens` in usage events.
...
- `auth.login.success` — published after successful login (API key or OAuth). Payload: `map[string]string{"provider": "<id>"}`. Subscribed by TUI to show success notifications and by agent loop to re-evaluate provider availability.
- `auth.logout` — published after logout. Payload: `map[string]string{"provider": "<id>"}`. Subscribed by agent loop to clear provider auth state.
...
On resume, the JSONL store filters out messages preceding the last compaction summary (messages starting with `[Compaction Summary]\n`). Only the summary and subsequent messages are restored, preventing the agent from re-processing summarized context.
