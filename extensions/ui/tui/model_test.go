...
func TestModel_OSCEscapeFiltered(t *testing.T) {
	m := newModel(nil, nil, nil)
	m.width = 80
	m.height = 24
	m.editor = m.editor.Focus()

	// Simulate OSC 11 background color response leaking as KeyRunes with Alt=true
	// (the leading \x1b sets the Alt flag in Bubbletea's parser)
	oscMsg := tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("]11;rgb:0000/0000/0000"), Alt: true}

	updated, cmd := m.Update(oscMsg)

	assert.Nil(t, cmd)
	// Editor should remain empty — OSC garbage not inserted
	assert.Empty(t, updated.(Model).editor.Value())
}

func TestModel_OSCEscapePastedTextNotFiltered(t *testing.T) {
	m := newModel(nil, nil, nil)
	m.width = 80
	m.height = 24
	m.editor = m.editor.Focus()

	// Pasted text starting with "]0;some title" has Alt=false and no color data
	// — should NOT be filtered (user could be typing an OSC example)
	pasteMsg := tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("]0;window title"), Alt: false}

	updated, cmd := m.Update(pasteMsg)

	assert.Nil(t, cmd)
	// Editor should contain the pasted text — not filtered
	assert.Contains(t, updated.(Model).editor.Value(), "]0;")
}

func TestModel_OSCTerminalColorResponseFiltered(t *testing.T) {
	m := newModel(nil, nil, nil)
	m.width = 80
	m.height = 24
	m.editor = m.editor.Focus()

	// Terminal color response without Alt (split-read edge case) — should be filtered
	colorMsg := tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("]11;rgb:0000/0000/0000"), Alt: false}

	updated, cmd := m.Update(colorMsg)

	assert.Nil(t, cmd)
	assert.Empty(t, updated.(Model).editor.Value())
}

func TestIsOSCEscape(t *testing.T) {
	tests := []struct {
		name  string
		runes []rune
		want  bool
	}{
		{"OSC 11 bg color", []rune("]11;rgb:0000/0000/0000"), true},
		{"OSC 10 fg color", []rune("10;rgb:aaaa/bbbb/cccc"), false}, // missing leading ]
		{"OSC 0 title", []rune("]0;window title"), true},
		{"normal text", []rune("hello world"), false},
		{"bracket no digits", []rune("]abc"), false},
		{"short", []rune("]1"), false},
		{"empty", []rune(""), false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, isOSCEscape(tt.runes))
		})
	}
}

func TestIsTerminalColorResponse(t *testing.T) {
	tests := []struct {
		name  string
		runes []rune
		want  bool
	}{
		{"rgb color", []rune("]11;rgb:0000/0000/0000"), true},
		{"hex color", []rune("]11;#abcdef"), true},
		{"title text", []rune("]0;window title"), false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, isTerminalColorResponse(tt.runes))
		})
	}
}
